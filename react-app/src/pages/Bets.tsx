...
function computePoints(bet: BetEntry, match: Match): number | null {
  ...
  const exactScore  = bet.homeScore === match.homeScore && bet.awayScore === match.awayScore;
  const rightWinner = Math.sign(bet.homeScore - bet.awayScore) === Math.sign(match.homeScore - match.awayScore);
...
export default function Bets() {
  ...
  const { data: matchData, loading: matchLoading } = useQuery<{ matches: Match[] }>(GET_MATCHES);
  const { data: betsData,  loading: betsLoading  } = useQuery<{ allBets: BetEntry[] }>(GET_ALL_BETS);
  ...
  const leaderboard = allBets.reduce<Map<string, { displayName: string; points: number }>>(
    (acc, bet) => {
      const match  = matchMap.get(bet.matchId);
      const points = match ? (computePoints(bet, match) ?? 0) : 0;
      const entry  = acc.get(bet.username) ?? { displayName: bet.displayName, points: 0 };
      entry.points += points;
  ...
  return (
    <>
      ...
      <h2 className="section-title" style={{ marginTop: 24 }}>Tous les paris</h2>
      {allBets.length === 0 ? (
          ...
          {allBets.map((bet, i) => {
            ...
            return (
              <li key={i} className="bet-item">
                <div>
                  <span className="bet-user">{bet.displayName || bet.username}</span>
                  {match && (
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                      <Link to={`/match/${bet.matchId}`}>{match.homeTeam} vs {match.awayTeam}</Link>
                    </div>
                  )}
                </div>
                <span className="bet-score">{bet.homeScore} – {bet.awayScore}</span>
              </li>
            );
          })}
        </ul>
