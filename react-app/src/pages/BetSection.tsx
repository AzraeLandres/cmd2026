  ...
  return (
    <section className="bet-section" aria-label="Paris des amis">
      ...
      {matchStatus !== 'FINISHED' && user && (
        <form className="bet-form" onSubmit={handleSubmit} aria-label="Placer mon pari">
          {match && (
            <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginRight: 6 }}>
              {match.homeTeam}
            </span>
          )}
          <input
      ...
      {saved     && <div className="form-success" role="status">Pari enregistré !</div>}

      {bets.length === 0 ? (
        <div className="muted" style={{ fontSize: '0.88rem', paddingBottom: 8 }}>
          ...
          {bets.map((bet, i) => {
            ...
            return (
              <li className="bet-item" key={i}>
                ...
                <span
                  className={'bet-score' + (result === 'exact' ? ' bet-correct' : '')}
