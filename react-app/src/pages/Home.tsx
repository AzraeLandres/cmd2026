...
function pickTeamMatch(matches: Match[], team: string): Match | null {
  return (
    matches.find((m) => (m.homeTeam === team || m.awayTeam === team) && m.status === 'LIVE') ??
    matches.find((m) => (m.homeTeam === team || m.awayTeam === team) && m.status === 'SCHEDULED') ??
    null
  );
}
...
export default function Home() {
  ...
  useEffect(() => {
    setHeader({ title: 'Coupe du Monde 2026', showBack: false, liveMinute: null });
  }, [setHeader]);

  const { data: matchData, loading: matchLoading, error: matchError } = useQuery<{ matches: Match[] }>(GET_MATCHES);
  const { data: betsData  } = useQuery<{ allBets: Bet[] }>(GET_ALL_BETS, { skip: !user });

  if (matchError) return <EmptyState role="alert">Impossible de charger les matchs.</EmptyState>;
  if (matchLoading) return <EmptyState>Chargement…</EmptyState>;

  const matches   = matchData?.matches ?? [];
  const myBets    = (betsData?.allBets ?? []).filter((b) => true);
  ...
  const upcomingBets = myBets
    ...
    .filter((b): b is (Bet & { match: Match }) => b !== null && b.match.status === 'SCHEDULED')
    .slice(0, 5);
  ...
  return (
    <>
      <section className="home-section" aria-label="Match du moment">
        ...
        {featured ? <MatchCard match={featured} /> : <EmptyState>Aucun match à afficher.</EmptyState>}
      </section>

      {favorites.map((team) => {
        const teamMatch = pickTeamMatch(matches, team);
        const isSameFeatured = featured && teamMatch && teamMatch.id === featured.id;
        return (
          <section className="home-section" key={team} aria-label={`Prochain match de ${team}`}>
            <h2 className="section-title">{team} — prochain match</h2>
            {!teamMatch          && <EmptyState>Aucun match trouvé pour {team}.</EmptyState>}
            {teamMatch && isSameFeatured  && <EmptyState>C'est le match affiché ci-dessus.</EmptyState>}
      ...
      {upcomingBets.length > 0 && (
        <section className="home-section" aria-label="Mes paris à venir">
          ...
          {upcomingBets.map(({ matchId, homeScore, awayScore, match: m }) => (
            <Link key={matchId} to={`/match/${matchId}`} className="card home-bet-card">
              <div className="home-bet-teams">
                <span>{m.homeTeam}</span>
                <span className="home-bet-score">{homeScore} – {awayScore}</span>
      ...
      {upcomingUnbetted.length > 0 && (
        <section className="home-section" aria-label="Matchs sans pari">
          ...
          {upcomingUnbetted.map((m) => (
            ...
            >
              <span style={{ fontWeight: 600, fontSize: '0.88rem' }}>{m.homeTeam} vs {m.awayTeam}</span>
              <span style={{ color: 'var(--primary)', fontSize: '0.82rem', fontWeight: 600 }}>Parier →</span>
            </Link>
          ))}
