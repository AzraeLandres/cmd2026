...
const { hashPassword, verifyPassword, createToken, verifyToken, getAuthUserId } = require('../lib/auth');
...
test('getAuthUserId lit le token depuis Authorization: Bearer', () => {
  const token = createToken(7);
  const req   = { headers: { authorization: 'Bearer ' + token } };
  assert.equal(getAuthUserId(req), 7);
});

test('getAuthUserId retourne null sans header', () => {
